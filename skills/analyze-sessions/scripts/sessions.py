"""Shared library for analyzing omp agent session logs.

A session is a JSONL file under ~/.omp/agent/sessions/<encoded-cwd>/, with
...
SESSIONS_ROOT = Path(os.path.expanduser("~/.omp/agent/sessions"))
    ...
    with f:
        for line in f:
            ...
            if t == "session":
                ...
                # omp model ids are "provider/model"
                mid = rec.get("model") or ""
                if "/" in mid:
...
def add_filter_args(p: argparse.ArgumentParser, *, subagents_default: bool) -> None:
    ...
    p.add_argument("--provider", metavar="NAME",
                   help="Restrict by provider (e.g. opencode-go, openai-codex).")
